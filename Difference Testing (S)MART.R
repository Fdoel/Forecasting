...
model.MART <- MART(inflation_df_monthly$inflationNonSA,NULL,1,1,0.6,1) # MART(1,1) with d=1
model.MAR <- marx.t(inflation_df_monthly$inflationNonSA,NULL,1,1) # MAR(1,11 model)

model.AR <- marx.t(inflation_df_monthly$inflationNonSA, NULL, 1, 0)
model.ART <- MART(inflation_df_monthly$inflationNonSA, NULL, 1,0, 0.6, 1) # MART(1,1) with d=1

### Invalid test due to nuissance parameter c. Would need to be done by simulation to get critical values
...
# Do the test for AR vs ART

F_ar <- t * (ar_resid_var - art_resid_var) / art_resid_var
p_value_ar <- 1 - pchisq(F_ar, df = 2 + 1)
